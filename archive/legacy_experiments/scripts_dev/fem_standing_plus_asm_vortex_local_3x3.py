...
3×3 trap grid region (~2λ wide).  a ``propagate_pressure_asm`` to every z-plane of interest.
...
def parse_args():
    ...
    p.add_argument("--load-standing", type=str, default=None,
                   metavar="PATH|latest",
                   help="Load cached standing-wave .npz instead of solving. "
                        "Use 'latest' to auto-detect newest cache file.")
    p.add_argument("--save-standing-only", action="store_true",
                   help="Run FEM solve, save cache, then exit (no overlay).")
    return p.parse_args()


def _load_standing_wave(path_or_latest: str):
    """Load a cached standing-wave solution from .npz."""
    if path_or_latest.lower() == "latest":
        candidates = sorted(CACHE_DIR.glob("standing_wave_epl*.npz"))
        if not candidates:
            raise FileNotFoundError(
                f"No standing wave cache found in {CACHE_DIR}")
        path = candidates[-1]
    else:
    ...
    coords = d["coords"]
    p_vals = d["p_real"] + 1j * d["p_imag"]
    meta = {}
    if "metadata" in d:
        meta = d["metadata"].item()
    return coords, p_vals, meta, path


def _save_standing_wave(coords, p_vals, cfg, solve_time, elem):
    """Save standing-wave DOF data to .npz cache."""
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    fname = f"standing_wave_epl{elem}.npz"
    path = CACHE_DIR / fname
    meta = {
        "elem_per_lam": elem,
        "dofs": len(p_vals),
        "max_pressure": float(np.abs(p_vals).max()),
    ...
    np.savez_compressed(path,
                        ...
                        metadata=meta)
    # Sidecar info
    info_path = CACHE_DIR / f"standing_wave_epl{elem}_INFO.txt"
    info_path.write_text(
        f"FEM Standing-Wave Cache\n"
        f"=======================\n"
        f"Created       : {meta['timestamp']}\n"
        f"elem/λ        : {elem}\n"
        f"DOFs          : {meta['dofs']}\n"
        f"Solve time    : {meta['solve_time']:.1f}s\n"
        f"max|p|        : {meta['max_pressure']:.4f} Pa\n"
    )
    return path
...
def _measure_trap_spacing_from_field(xg, yg, field_mag):
    ...
    # Minimum distance between peaks ≈ 0.7 × λ/2 in grid indices
    dx = xg[1] - xg[0]
    min_dist = max(5, int(0.7 * TRAP_SPACING / dx))
    peaks, _ = find_peaks(profile_x, height=0.3, distance=min_dist)
    print(f"    [trap_spacing debug] dx={dx*1e6:.1f}µm, min_dist={min_dist}px, "
          f"n_peaks={len(peaks)}, peak_idx={peaks.tolist()}", flush=True)
    if len(peaks) < 2:
        return np.nan
...
def main():
    ...
    # ─────────────────────────────────────────────────────────────
    # STEP 1: Load or solve FEM standing-wave-only
    # ─────────────────────────────────────────────────────────────
    log("─" * 72)
    ...
    if args.load_standing:
        # ── Load from cache ───────────────────────────────────────
        coords_fem, pv_fem, cache_meta, cache_path = _load_standing_wave(
            args.load_standing)
        log(f"  Loaded {cache_path.name}")
        log(f"  DOFs = {len(pv_fem)}")
        log(f"  max|p_stand| = {np.abs(pv_fem).max():.2f} Pa")
        if cache_meta:
            log(f"  (cached at {cache_meta.get('elem_per_lam', '?')} elem/λ, "
                f"solve time {cache_meta.get('solve_time', '?')}s)")
    else:
        # ── Fresh FEM solve ────────────────────────────────────────
        fem_overrides = {
            **CORRECTED_PRESET,
        ...
        H_TOTAL = sol_stand.cfg.H_total
        del sol_stand
        gc.collect()

        # Optionally save cache and exit
        if args.save_standing_only:
            cp = _save_standing_wave(coords_fem, pv_fem, None, t_fem, ELEM)
            log(f"  Saved cache: {cp}")
            log("  --save-standing-only: exiting.")
            return
    log("")
    ...
    # Filter to a thin z-slab around z* for 2-D Delaunay (fast).
    # 3-D LinearNDInterpolator on 762K+ DOFs is catastrophically slow.
    # Estimate element size from actual DOF coordinates (works with cache).
    h_elem = LAM / ELEM  # fallback
    if len(coords_fem) > 100:
        # Estimate from z-spacing of sorted unique z-coords near z*
        z_near = coords_fem[:, 2]
        z_uniq = np.unique(z_near)
        dz_est = np.median(np.diff(z_uniq))
        if dz_est > 0:
            h_elem = float(dz_est)
    slab_tol = 3.0 * h_elem
    z_all = coords_fem[:, 2]
    slab_mask = np.abs(z_all - Z_STAR) < slab_tol
    coords_slab = coords_fem[slab_mask][:, :2]  # (x, y) only
    pv_slab = pv_fem[slab_mask]
    log(f"  XY slab: |z - z*| < {slab_tol*1e3:.3f} mm → "
        f"{slab_mask.sum()} / {len(pv_fem)} DOFs")
    log("  Interpolating FEM → local grid (2-D LinearNDInterpolator) ...")
    t0 = time.time()
    ...
    log(f"  XY interpolation: {time.time() - t0:.1f}s")
    ...
    # 4a. Trap spacing
    print("[DEBUG] about to call _measure_trap_spacing", flush=True)
    trap_sp_meas = _measure_trap_spacing_from_field(
        xg_loc, yg_loc, np.abs(p_stand_local))
    print(f"[DEBUG] trap_sp_meas = {trap_sp_meas}", flush=True)
    ...
    # Filter to thin y-slab for 2-D Delaunay in (x, z)
    y_all = coords_fem[:, 1]
    yslab_mask = np.abs(y_all - CY) < slab_tol
    coords_yslab = coords_fem[yslab_mask][:, [0, 2]]  # (x, z)
    pv_yslab = pv_fem[yslab_mask]
    log(f"  XZ slab: |y - CY| < {slab_tol*1e3:.3f} mm → "
        f"{yslab_mask.sum()} / {len(pv_fem)} DOFs")
    log("  Interpolating FEM → XZ slice (2-D LinearNDInterpolator) ...")
    t0 = time.time()
    ...
    zg_xz = np.linspace(H_UNDER * 0.5, H_TOTAL, NZ_MERID)
    ...
    log(f"  XZ interpolation: {time.time() - t0:.1f}s")
