#!/usr/bin/env python3
"""
Authoritative COMSOL Comparison Rebuild — Attempt 4.

Fix from Attempt 3:
  ALL cases now use disc_robin=False. The disc is a PURE NEUMANN velocity
  source (no impedance Robin term). The impedance Robin term absorbed
  standing-wave energy inside the disc region in ALL modes (not just Case A).

  Diagnostic proof (disc_neumann_vs_robin investigation):
    - Case C V₀ Robin:   avg|p|_disc = 11.16 Pa (48.5% of standing)
    - Case C V₀ Neumann: avg|p|_disc = 25.07 Pa (108.9% of standing)
    - Case B Neumann:    max|p| = 35.81 Pa (vs 10.88 Robin) — 3.3× stronger

Physics:
  Case A — standing-only: rigid bottom everywhere (∂p/∂n = 0 on disc)
  Case B — vortex-only:   pure Neumann source on disc (no impedance)
  Case C — combined:      pure Neumann source on disc + standing walls
...
# *** FIX (Attempt 4): ALL cases use disc_robin=False ***
# The disc is a PURE VELOCITY SOURCE (Neumann), NOT an impedance surface.
# The impedance Robin BC was absorbing standing-wave energy inside the disc
# region in all modes, creating a "hole" in the pressure field.
DISC_ROBIN = {
    "Case_A_standing": False,  # rigid bottom — no disc impedance, no vortex
    "Case_B_vortex": False,    # pure Neumann vortex source (no impedance)
    "Case_C_combined": False,  # pure Neumann vortex source + standing walls
}
...
def write_readme(cfg, diag_lines, result_table):
    ...
    **Key change:** The disc has NO impedance Robin term. It is a pure velocity
    ...
    | ρ | 997 kg/m³ |
    | c | 1484 m/s |
    | Z_w = ρc | {cfg.Z_water:.0f} Pa·s/m (NOT used for disc BC) |
...
def main():
    ...
    log(f"\n{'#'*70}")
    log(f"  COMSOL COMPARISON REBUILD — ATTEMPT 4")
    log(f"  Fix: ALL cases disc_robin=False (pure Neumann source)")
    log(f"  Disc is a pure velocity source, NOT impedance absorber")
    ...
    log(f"\n{'#'*70}")
    log("  Attempt 4 COMSOL Comparison Built Successfully")
