...
Same physics, field construction, scoring, and A/B dynamics as
vortex_entry_hires_diagnostic.py.  Adds an optional ensemble of inertial
polystyrene particles (100 µm diameter) governed by:

    dx/dt = v
    dv/dt = ACCEL_SCALE * F_norm(x)  -  v / TAU_STOKES

where F_norm is the normalised Gor'kov force (same iFx, iFy as A/B dynamics).
A/B dynamics are UNCHANGED.  GIF and proximity graph are UNCHANGED.
Particle ensemble output is written to a separate particles.csv.

Outputs:
    results/dev/vortex_entry_hires_inertial/<timestamp>/
        vortex_entry_hires_<label>.gif        — identical to diagnostic version
        timeseries.csv                        — identical to diagnostic version
        proximity_vs_time.png                 — identical to diagnostic version
        particles.csv                         — NEW: inertial ensemble snapshots
        manifest.json                         — extends diagnostic manifest

Usage:
    python scripts/dev/vortex_entry_hires_inertial.py
    python scripts/dev/vortex_entry_hires_inertial.py --particle_grid_n 10
    python scripts/dev/vortex_entry_hires_inertial.py --particle_grid_n 15 --n_psi 36

Particle system is disabled by default (--particle_grid_n 0).
With --particle_grid_n N, an N×N regular grid is placed over the view window.
"""
...
# ── Defaults (same successful case as vortex_entry_hires_diagnostic.py) ──
...
PARTICLE_GRID_N_DEFAULT  = 0          # 0 = disabled; N → N×N grid
...
# ══════════════════════════════════════════════════════════════════
# View + rendering  (identical to vortex_entry_hires_diagnostic.py)
# ══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════
# Proximity-vs-time plot  (identical to vortex_entry_hires_diagnostic.py)
# ══════════════════════════════════════════════════════════════════
...
def run_experiment(args) -> None:
    ...
    # ── Snapshot schedule (informational) ────────────────────────
    ...
    if particles_active:
        ...
        print(f"    τ_Stokes    = {TAU_STOKES*1e3:.3f} ms  "
              f"(≈ {TAU_STOKES / 1e-4:.1f} × DT_DEFAULT)")
        print(f"    ACCEL_SCALE = {ACCEL_SCALE:.1f} m/s²  per  (Pa⁻¹ m⁻¹)")
        print(f"    integrator  = {args.particle_integrator}")
        print(f"    initial vel = zero")
    else:
        print(f"\n  Inertial particle ensemble: DISABLED "
              f"(use --particle_grid_n N to enable)")

    # ── Output directory ──────────────────────────────────────────
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = (PROJECT_ROOT / "results" / "dev"
               / "vortex_entry_hires_inertial" / timestamp)
    ...
    # Particles stay at initial grid positions during intro — no iFx/iFy
    # available until the first phase_sweep at v_step=0.
    ...
    for v_step in range(n_path):
        ...
        for dyn_i in range(n_dyn):
            # ── A/B overdamped dynamics (UNCHANGED) ───────────────
            pos_A, pos_B = update_particles(
                pos_A, pos_B, iFx, iFy,
                args.max_step, xg, yg,
            )

            # ── Inertial ensemble (separate, does not affect A/B) ─
            if particles_active:
    ...
    if args.n_hold > 0:
        ...
        for hi in range(args.n_hold):
            ...
            if particles_active:
                ensemble = advance_ensemble(
                    ensemble, iFx_hold, iFy_hold, xg, yg,
                    integrator=args.particle_integrator,
                )
    ...
    if particles_active:
        ...
        print(f"  Saved: {particle_csv} "
              f"({particle_recorder.n_snapshots} snapshots × "
              f"{ensemble.N} particles = "
              f"{len(particle_recorder.rows)} rows)")
    ...
    manifest = {
        ...
        "script": "vortex_entry_hires_inertial.py",
        "alpha": alpha,
        ...
        "particle_diameter_um": PARTICLE_DIAMETER * 1e6,
        "particle_tau_ms": round(TAU_STOKES * 1e3, 4),
        "particle_accel_scale": round(ACCEL_SCALE, 2),
        "particle_integrator": args.particle_integrator,
        "particle_n_snapshots": (particle_recorder.n_snapshots
                                  if particles_active else 0),
    }
    ...
    print(f"\nDone in {elapsed:.1f}s — {len(frames_data)} GIF frames, "
          ...
          + (f", {len(particle_recorder.rows)} particle rows"
             if particles_active else ""))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Vortex entry — high-res diagnostic with inertial particles")
    ...
    g.add_argument("--particle_grid_n", type=int,
                   ...
                   help="Grid side length for inertial particle ensemble. "
                        "0 = disabled.  N creates an N×N grid of 100 µm "
                        "polystyrene spheres over the view window. "
                        "Output: particles.csv  (default %(default)s)")
    g.add_argument("--particle_integrator", type=str,
                   default=PARTICLE_INTEGRATOR_DEFAULT,
                   choices=["rk4", "euler"],
                   help="Time integrator for inertial particles. "
                        "'rk4' is recommended (default %(default)s)")
