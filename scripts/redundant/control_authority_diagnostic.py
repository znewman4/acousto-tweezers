#!/usr/bin/env python3
"""
Control Authority Diagnostic Experiment

PURPOSE:
Quantify which control knobs (macro actions) actually matter for shaping
the force field at the particle. This diagnostic directly informs which
actions should be kept, pruned, merged, or expanded before any further
optimization or MPC work.

METHODOLOGY:
From a fixed baseline state:
1. For each macro action, apply it once and solve the PDE
2. Measure local changes (force at particle) and global changes (field statistics)
3. Produce ranked reports showing which actions have real control authority

OUTPUTS:
results/control_authority/run_YYYYMMDD_HHMMSS/
    - control_authority.csv: One row per action with all metrics
    - summary_bars.png: Bar charts ranking actions
    - printed summary at end

Usage:
    python scripts/control_authority_diagnostic.py
    python scripts/control_authority_diagnostic.py --fast
    python scripts/control_authority_diagnostic.py --directions 4
    python scripts/control_authority_diagnostic.py --particle_x 1.0 --particle_y 1.0
"""

from __future__ import annotations

import matplotlib
matplotlib.use("Agg")

import argparse
import csv
from dataclasses import dataclass, field as dataclass_field
from datetime import datetime
from pathlib import Path
import sys
import time
import numpy as np
import matplotlib.pyplot as plt
from typing import Optional

# Add project root to path
_REPO = Path(__file__).resolve().parents[1]
if str(_REPO) not in sys.path:
    sys.path.insert(0, str(_REPO))

from acousto.force import ParticleProps, gorkov_potential_and_force_2d, bilinear_sample_vec
from acousto.analysis import find_trap_center

from tweezers.control import (
    DishDomain, MediumProps, EvaluatorConfig,
    Control3Pucks,
)
from tweezers.control.evaluator_3pucks import Evaluator3Pucks

from scripts.macro_actions_3puck import (
    MacroAction3Puck,
    MacroActionType3Puck,
    apply_macro_action_3puck,
)


# =============================================================================
# Data structures for metrics
# =============================================================================

@dataclass
class ActionMetrics:
    """Comprehensive metrics for one action's control authority."""
    action_name: str
    
    # Local metrics (at particle position)
    baseline_force_mag: float       # |F_baseline| at particle
    delta_force_mag: float          # |F_action| - |F_baseline|
    delta_force_mag_rel: float      # relative change in force magnitude
    delta_force_x: float            # F_action_x - F_baseline_x
    delta_force_y: float            # F_action_y - F_baseline_y
    
    # Directional control (fixed-direction projections)
    delta_force_proj_x: float       # Δ(F · x̂) - projection onto +x
    delta_force_proj_y: float       # Δ(F · ŷ) - projection onto +y
    max_delta_force_proj: float     # Max Δ(F · d̂) - best direction
    best_direction_deg: float       # Direction that gives max_delta_force_proj
    
    # Global field metrics
    baseline_U_std: float           # std(U) for baseline
    delta_U_std: float              # std(U_action) - std(U_baseline)
    baseline_U_range_p95_p5: float  # p95 - p5 for baseline U
    delta_U_range: float            # (p95 - p5)_action - (p95 - p5)_baseline
    
    # Trap metrics
    baseline_trap_present: bool
    action_trap_present: bool
    trap_appeared: bool             # trap not present -> present
    trap_disappeared: bool          # trap present -> not present
    delta_stiffness_min: float      # change in min eigenvalue (if both present)
    delta_trap_x: float             # trap position change
    delta_trap_y: float
    
    # Solver time
    solver_time_ms: float


@dataclass
class BaselineState:
    """Baseline state from which all actions are compared."""
    particle_x: float
    particle_y: float
    control: Control3Pucks
    
    # Baseline field solution
    field: object  # HelmholtzField2D
    U: np.ndarray
    Fx: np.ndarray
    Fy: np.ndarray
    Fx_scaled: np.ndarray
    Fy_scaled: np.ndarray
    
    # Force at particle
    fx_at_particle: float
    fy_at_particle: float
    force_mag_at_particle: float
    
    # Field statistics
    U_std: float
    U_p5: float
    U_p95: float
    U_range_p95_p5: float
    
    # Trap info (may not exist)
    trap_present: bool
    trap_x: float
    trap_y: float
    stiffness_min: float


# =============================================================================
# Core evaluation functions
# =============================================================================

def compute_baseline(
    ev: Evaluator3Pucks,
    particle_x: float,
    particle_y: float,
    ctrl: Control3Pucks,
) -> BaselineState:
    """Solve baseline field and extract all reference metrics."""
    
    # Solve PDE
    vb = ev.control_to_forcing_band_vb(ctrl)
    field = ev.op.solve_for_bottom_vb(vb)
    U, Fx, Fy = gorkov_potential_and_force_2d(field, ev.particle)
    
    # Apply scaling
    Fx_scaled = Fx * ev.cfg.alpha_g
    Fy_scaled = Fy * ev.cfg.alpha_g
    
    # Force at particle
    fx, fy = bilinear_sample_vec(field.x, field.y, Fx_scaled, Fy_scaled, particle_x, particle_y)
    force_mag = np.sqrt(fx**2 + fy**2)
    
    # Field statistics
    U_flat = U.flatten()
    U_std = float(np.std(U_flat))
    U_p5 = float(np.percentile(U_flat, 5))
    U_p95 = float(np.percentile(U_flat, 95))
    
    # Trap detection
    trap = find_trap_center(
        field.x, field.y, U, Fx, Fy,
        particle_x=particle_x, particle_y=particle_y,
        search_radius=0.5e-3,
    )
    trap_present = trap.is_stable
    trap_x = trap.x if trap.x is not None else np.nan
    trap_y = trap.y if trap.y is not None else np.nan
    stiffness_min = float(np.min(trap.stiffness_eigvals)) if trap.stiffness_eigvals is not None else np.nan
    
    return BaselineState(
        particle_x=particle_x,
        particle_y=particle_y,
        control=ctrl,
        field=field,
        U=U,
        Fx=Fx,
        Fy=Fy,
        Fx_scaled=Fx_scaled,
        Fy_scaled=Fy_scaled,
        fx_at_particle=fx,
        fy_at_particle=fy,
        force_mag_at_particle=force_mag,
        U_std=U_std,
        U_p5=U_p5,
        U_p95=U_p95,
        U_range_p95_p5=U_p95 - U_p5,
        trap_present=trap_present,
        trap_x=trap_x,
        trap_y=trap_y,
        stiffness_min=stiffness_min,
    )


def evaluate_action(
    ev: Evaluator3Pucks,
    baseline: BaselineState,
    action_type: MacroActionType3Puck,
    macro_magnitude: float,
    macro_phase_step: float,
    macro_amplitude_step: float,
    test_directions: np.ndarray,  # (N, 2) unit vectors
) -> ActionMetrics:
    """Evaluate one action's control authority relative to baseline."""
    
    t0 = time.perf_counter()
    
    # Create and apply action
    action = MacroAction3Puck(
        action_type=action_type,
        magnitude=macro_magnitude,
        phase_step=macro_phase_step,
        amplitude_step=macro_amplitude_step,
    )
    u_new = apply_macro_action_3puck(baseline.control, action)
    u_new = ev.clip_control(u_new)
    
    # Solve PDE for new control
    vb = ev.control_to_forcing_band_vb(u_new)
    field = ev.op.solve_for_bottom_vb(vb)
    U, Fx, Fy = gorkov_potential_and_force_2d(field, ev.particle)
    
    Fx_scaled = Fx * ev.cfg.alpha_g
    Fy_scaled = Fy * ev.cfg.alpha_g
    
    solver_time_ms = (time.perf_counter() - t0) * 1000.0
    
    # Force at particle
    fx, fy = bilinear_sample_vec(
        field.x, field.y, Fx_scaled, Fy_scaled,
        baseline.particle_x, baseline.particle_y
    )
    force_mag = np.sqrt(fx**2 + fy**2)
    
    # Delta force
    delta_fx = fx - baseline.fx_at_particle
    delta_fy = fy - baseline.fy_at_particle
    delta_force_mag = force_mag - baseline.force_mag_at_particle
    delta_force_mag_rel = delta_force_mag / (baseline.force_mag_at_particle + 1e-15)
    
    # Directional control: compute Δ(F · d̂) for each test direction
    delta_force_projs = []
    for d_hat in test_directions:
        # Baseline projection
        proj_baseline = baseline.fx_at_particle * d_hat[0] + baseline.fy_at_particle * d_hat[1]
        # Action projection
        proj_action = fx * d_hat[0] + fy * d_hat[1]
        # Delta
        delta_force_projs.append(proj_action - proj_baseline)
    
    delta_force_projs = np.array(delta_force_projs)
    # Fixed-direction projections (more meaningful than avg which cancels to ~0)
    delta_force_proj_x = float(delta_fx)  # Projection onto +x
    delta_force_proj_y = float(delta_fy)  # Projection onto +y
    max_idx = int(np.argmax(delta_force_projs))
    max_delta_force_proj = float(delta_force_projs[max_idx])
    best_direction_deg = float(np.degrees(np.arctan2(test_directions[max_idx, 1], test_directions[max_idx, 0])))
    
    # Field statistics
    U_flat = U.flatten()
    U_std = float(np.std(U_flat))
    U_p5 = float(np.percentile(U_flat, 5))
    U_p95 = float(np.percentile(U_flat, 95))
    
    delta_U_std = U_std - baseline.U_std
    delta_U_range = (U_p95 - U_p5) - baseline.U_range_p95_p5
    
    # Trap detection
    trap = find_trap_center(
        field.x, field.y, U, Fx, Fy,
        particle_x=baseline.particle_x, particle_y=baseline.particle_y,
        search_radius=0.5e-3,
    )
    action_trap_present = trap.is_stable
    action_trap_x = trap.x if trap.x is not None else np.nan
    action_trap_y = trap.y if trap.y is not None else np.nan
    action_stiffness_min = float(np.min(trap.stiffness_eigvals)) if trap.stiffness_eigvals is not None else np.nan
    
    # Trap changes
    trap_appeared = (not baseline.trap_present) and action_trap_present
    trap_disappeared = baseline.trap_present and (not action_trap_present)
    
    if baseline.trap_present and action_trap_present:
        delta_stiffness_min = action_stiffness_min - baseline.stiffness_min
        delta_trap_x = action_trap_x - baseline.trap_x
        delta_trap_y = action_trap_y - baseline.trap_y
    else:
        delta_stiffness_min = np.nan
        delta_trap_x = np.nan
        delta_trap_y = np.nan
    
    return ActionMetrics(
        action_name=action_type.name,
        baseline_force_mag=baseline.force_mag_at_particle,
        delta_force_mag=delta_force_mag,
        delta_force_mag_rel=delta_force_mag_rel,
        delta_force_x=delta_fx,
        delta_force_y=delta_fy,
        delta_force_proj_x=delta_force_proj_x,
        delta_force_proj_y=delta_force_proj_y,
        max_delta_force_proj=max_delta_force_proj,
        best_direction_deg=best_direction_deg,
        baseline_U_std=baseline.U_std,
        delta_U_std=delta_U_std,
        baseline_U_range_p95_p5=baseline.U_range_p95_p5,
        delta_U_range=delta_U_range,
        baseline_trap_present=baseline.trap_present,
        action_trap_present=action_trap_present,
        trap_appeared=trap_appeared,
        trap_disappeared=trap_disappeared,
        delta_stiffness_min=delta_stiffness_min,
        delta_trap_x=delta_trap_x,
        delta_trap_y=delta_trap_y,
        solver_time_ms=solver_time_ms,
    )


# =============================================================================
# Output generation
# =============================================================================

def write_csv(metrics: list[ActionMetrics], path: Path) -> None:
    """Write metrics to CSV file."""
    fieldnames = [
        "action_name",
        "delta_force_mag",
        "delta_force_mag_rel",
        "delta_force_x",
        "delta_force_y",
        "delta_force_proj_x",
        "delta_force_proj_y",
        "max_delta_force_proj",
        "best_direction_deg",
        "delta_U_std",
        "delta_U_range",
        "baseline_trap_present",
        "action_trap_present",
        "trap_appeared",
        "trap_disappeared",
        "delta_stiffness_min",
        "delta_trap_x",
        "delta_trap_y",
        "solver_time_ms",
    ]
    
    with open(path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        for m in metrics:
            row = {
                "action_name": m.action_name,
                "delta_force_mag": f"{m.delta_force_mag:.6e}",
                "delta_force_mag_rel": f"{m.delta_force_mag_rel:.4f}",
                "delta_force_x": f"{m.delta_force_x:.6e}",
                "delta_force_y": f"{m.delta_force_y:.6e}",
                "delta_force_proj_x": f"{m.delta_force_proj_x:.6e}",
                "delta_force_proj_y": f"{m.delta_force_proj_y:.6e}",
                "max_delta_force_proj": f"{m.max_delta_force_proj:.6e}",
                "best_direction_deg": f"{m.best_direction_deg:.1f}",
                "delta_U_std": f"{m.delta_U_std:.6e}",
                "delta_U_range": f"{m.delta_U_range:.6e}",
                "baseline_trap_present": str(m.baseline_trap_present),
                "action_trap_present": str(m.action_trap_present),
                "trap_appeared": str(m.trap_appeared),
                "trap_disappeared": str(m.trap_disappeared),
                "delta_stiffness_min": f"{m.delta_stiffness_min:.6e}" if not np.isnan(m.delta_stiffness_min) else "nan",
                "delta_trap_x": f"{m.delta_trap_x:.6e}" if not np.isnan(m.delta_trap_x) else "nan",
                "delta_trap_y": f"{m.delta_trap_y:.6e}" if not np.isnan(m.delta_trap_y) else "nan",
                "solver_time_ms": f"{m.solver_time_ms:.1f}",
            }
            writer.writerow(row)


def plot_summary_bars(metrics: list[ActionMetrics], baseline: BaselineState, path: Path) -> None:
    """Create bar charts ranking actions by various metrics."""
    
    # Sort by different metrics for different panels
    names = [m.action_name for m in metrics]
    n_actions = len(metrics)
    
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(
        f"Control Authority Diagnostic\n"
        f"Particle at ({baseline.particle_x*1e3:.2f}, {baseline.particle_y*1e3:.2f}) mm, "
        f"Baseline |F| = {baseline.force_mag_at_particle:.2e} N",
        fontsize=12,
    )
    
    colors = plt.cm.tab20(np.linspace(0, 1, n_actions))
    
    # Panel 1: Force magnitude change
    ax1 = axes[0, 0]
    delta_mags = [m.delta_force_mag for m in metrics]
    sorted_idx = np.argsort(delta_mags)[::-1]
    sorted_names = [names[i] for i in sorted_idx]
    sorted_vals = [delta_mags[i] for i in sorted_idx]
    sorted_colors = [colors[i] for i in sorted_idx]
    
    bars = ax1.barh(range(n_actions), sorted_vals, color=sorted_colors)
    ax1.set_yticks(range(n_actions))
    ax1.set_yticklabels(sorted_names, fontsize=8)
    ax1.set_xlabel("Δ|F| (N)")
    ax1.set_title("Force Magnitude Change")
    ax1.axvline(x=0, color='k', linestyle='-', linewidth=0.5)
    ax1.invert_yaxis()
    
    # Panel 2: Max directional control
    ax2 = axes[0, 1]
    max_projs = [m.max_delta_force_proj for m in metrics]
    sorted_idx = np.argsort(max_projs)[::-1]
    sorted_names = [names[i] for i in sorted_idx]
    sorted_vals = [max_projs[i] for i in sorted_idx]
    sorted_colors = [colors[i] for i in sorted_idx]
    
    bars = ax2.barh(range(n_actions), sorted_vals, color=sorted_colors)
    ax2.set_yticks(range(n_actions))
    ax2.set_yticklabels(sorted_names, fontsize=8)
    ax2.set_xlabel("Max Δ(F·d̂) (N)")
    ax2.set_title("Best Directional Control")
    ax2.axvline(x=0, color='k', linestyle='-', linewidth=0.5)
    ax2.invert_yaxis()
    
    # Panel 3: Gor'kov potential std change
    ax3 = axes[1, 0]
    delta_stds = [m.delta_U_std for m in metrics]
    sorted_idx = np.argsort(np.abs(delta_stds))[::-1]
    sorted_names = [names[i] for i in sorted_idx]
    sorted_vals = [delta_stds[i] for i in sorted_idx]
    sorted_colors = [colors[i] for i in sorted_idx]
    
    bars = ax3.barh(range(n_actions), sorted_vals, color=sorted_colors)
    ax3.set_yticks(range(n_actions))
    ax3.set_yticklabels(sorted_names, fontsize=8)
    ax3.set_xlabel("Δstd(U)")
    ax3.set_title("Global Field Change (|Δstd(U)|)")
    ax3.axvline(x=0, color='k', linestyle='-', linewidth=0.5)
    ax3.invert_yaxis()
    
    # Panel 4: Aggregate control authority score
    # Combine metrics: |Δ|F|| + |max Δ(F·d̂)| + |Δstd(U)| (normalized)
    ax4 = axes[1, 1]
    
    # Normalize each metric to [0, 1] for fair comparison
    abs_delta_mags = np.abs(delta_mags)
    abs_max_projs = np.abs(max_projs)
    abs_delta_stds = np.abs(delta_stds)
    
    norm_mags = abs_delta_mags / (np.max(abs_delta_mags) + 1e-15)
    norm_projs = abs_max_projs / (np.max(abs_max_projs) + 1e-15)
    norm_stds = abs_delta_stds / (np.max(abs_delta_stds) + 1e-15)
    
    aggregate_scores = norm_mags + norm_projs + norm_stds
    
    sorted_idx = np.argsort(aggregate_scores)[::-1]
    sorted_names = [names[i] for i in sorted_idx]
    sorted_vals = [aggregate_scores[i] for i in sorted_idx]
    sorted_colors = [colors[i] for i in sorted_idx]
    
    bars = ax4.barh(range(n_actions), sorted_vals, color=sorted_colors)
    ax4.set_yticks(range(n_actions))
    ax4.set_yticklabels(sorted_names, fontsize=8)
    ax4.set_xlabel("Aggregate Score (normalized)")
    ax4.set_title("Combined Control Authority")
    ax4.invert_yaxis()
    
    plt.tight_layout()
    plt.savefig(path, dpi=150)
    plt.close()


def plot_direction_polar(metrics: list[ActionMetrics], path: Path) -> None:
    """Create polar plot showing best control direction for each action."""
    
    fig, ax = plt.subplots(1, 1, figsize=(10, 10), subplot_kw=dict(projection='polar'))
    
    # Filter to actions with significant control (exclude HOLD and near-zero)
    significant = [m for m in metrics if m.action_name != "HOLD" and abs(m.max_delta_force_proj) > 1e-15]
    
    if not significant:
        plt.close()
        return
    
    colors = plt.cm.tab20(np.linspace(0, 1, len(significant)))
    
    for i, m in enumerate(significant):
        angle_rad = np.radians(m.best_direction_deg)
        magnitude = m.max_delta_force_proj
        
        ax.annotate(
            '',
            xy=(angle_rad, abs(magnitude)),
            xytext=(0, 0),
            arrowprops=dict(
                arrowstyle='->', 
                color=colors[i],
                lw=2,
            ),
        )
        ax.annotate(
            m.action_name,
            xy=(angle_rad, abs(magnitude)),
            fontsize=7,
            ha='center',
        )
    
    ax.set_title("Best Control Direction per Action\n(arrow length = |Δ(F·d̂)|)")
    
    plt.tight_layout()
    plt.savefig(path, dpi=150)
    plt.close()


def print_summary(metrics: list[ActionMetrics], baseline: BaselineState, top_k: int = 5) -> None:
    """Print interpretable summary to stdout."""
    
    print("\n" + "=" * 70)
    print("CONTROL AUTHORITY DIAGNOSTIC - SUMMARY")
    print("=" * 70)
    
    print(f"\nBaseline State:")
    print(f"  Particle position: ({baseline.particle_x*1e3:.2f}, {baseline.particle_y*1e3:.2f}) mm")
    print(f"  Force at particle: ({baseline.fx_at_particle:.2e}, {baseline.fy_at_particle:.2e}) N")
    print(f"  Force magnitude: {baseline.force_mag_at_particle:.2e} N")
    print(f"  Trap present: {baseline.trap_present}")
    if baseline.trap_present:
        print(f"    Trap position: ({baseline.trap_x*1e3:.3f}, {baseline.trap_y*1e3:.3f}) mm")
        print(f"    Min stiffness: {baseline.stiffness_min:.2e}")
    print(f"  Gor'kov U std: {baseline.U_std:.2e}")
    print(f"  Gor'kov U range (p95-p5): {baseline.U_range_p95_p5:.2e}")
    
    # Rank by max directional control (the most operationally relevant metric)
    by_dir_ctrl = sorted(metrics, key=lambda m: m.max_delta_force_proj, reverse=True)
    
    print(f"\n--- Top {top_k} Actions by DIRECTIONAL CONTROL (max Δ(F·d̂)) ---")
    for i, m in enumerate(by_dir_ctrl[:top_k]):
        print(f"  {i+1}. {m.action_name:30s}  Δ(F·d̂) = {m.max_delta_force_proj:+.2e} N  @ {m.best_direction_deg:+6.1f}°")
    
    # Rank by force magnitude change
    by_force_mag = sorted(metrics, key=lambda m: abs(m.delta_force_mag), reverse=True)
    
    print(f"\n--- Top {top_k} Actions by FORCE MAGNITUDE CHANGE (|Δ|F||) ---")
    for i, m in enumerate(by_force_mag[:top_k]):
        print(f"  {i+1}. {m.action_name:30s}  Δ|F| = {m.delta_force_mag:+.2e} N  ({m.delta_force_mag_rel:+.1%})")
    
    # Near-zero effect actions (candidates for pruning)
    threshold = 0.05 * max(abs(m.max_delta_force_proj) for m in metrics)
    low_effect = [m for m in metrics if abs(m.max_delta_force_proj) < threshold]
    
    print(f"\n--- Actions with NEAR-ZERO Effect (candidates for pruning) ---")
    if low_effect:
        for m in low_effect:
            print(f"  - {m.action_name:30s}  Δ(F·d̂) = {m.max_delta_force_proj:+.2e} N")
    else:
        print("  (None - all actions have significant effect)")
    
    # Trap effects
    trap_changers = [m for m in metrics if m.trap_appeared or m.trap_disappeared]
    print(f"\n--- Actions that AFFECT TRAP EXISTENCE ---")
    if trap_changers:
        for m in trap_changers:
            if m.trap_appeared:
                print(f"  - {m.action_name}: Trap APPEARED")
            if m.trap_disappeared:
                print(f"  - {m.action_name}: Trap DISAPPEARED")
    else:
        print("  (None - trap state unchanged by all actions)")
    
    # Identify redundant pairs (opposite actions with similar magnitude)
    print(f"\n--- REDUNDANT ACTION PAIRS (symmetric effects) ---")
    action_pairs = [
        ("TRANSLATE_TRAP_X_POS", "TRANSLATE_TRAP_X_NEG"),
        ("TRANSLATE_TRAP_Y_POS", "TRANSLATE_TRAP_Y_NEG"),
        ("ROTATE_INTERFERENCE_CW", "ROTATE_INTERFERENCE_CCW"),
        ("MOVE_A_RIGHT", "MOVE_A_LEFT"),
        ("MOVE_B_RIGHT", "MOVE_B_LEFT"),
        ("MOVE_C_UP", "MOVE_C_DOWN"),
        ("PHASE_SHIFT_B_POS", "PHASE_SHIFT_B_NEG"),
        ("PHASE_SHIFT_C_POS", "PHASE_SHIFT_C_NEG"),
        ("WIDEN", "NARROW"),
        ("STRENGTHEN_TRAP", "WEAKEN_TRAP"),
    ]
    
    name_to_metric = {m.action_name: m for m in metrics}
    
    for a1, a2 in action_pairs:
        if a1 in name_to_metric and a2 in name_to_metric:
            m1 = name_to_metric[a1]
            m2 = name_to_metric[a2]
            # Check if they have opposite force effects (symmetric)
            if abs(m1.delta_force_x + m2.delta_force_x) < 0.1 * max(abs(m1.delta_force_x), abs(m2.delta_force_x), 1e-15):
                # Force effects cancel → symmetric pair
                avg_effect = 0.5 * (abs(m1.max_delta_force_proj) + abs(m2.max_delta_force_proj))
                print(f"  {a1} ↔ {a2}: symmetric, avg effect = {avg_effect:.2e} N")
    
    # Implications
    print(f"\n--- IMPLICATIONS ---")
    
    # Robot motion vs field modulation
    position_actions = [m for m in metrics if any(x in m.action_name for x in ["TRANSLATE", "MOVE", "WIDEN", "NARROW"])]
    phase_actions = [m for m in metrics if any(x in m.action_name for x in ["PHASE", "ROTATE"])]
    
    avg_pos_effect = np.mean([abs(m.max_delta_force_proj) for m in position_actions]) if position_actions else 0
    avg_phase_effect = np.mean([abs(m.max_delta_force_proj) for m in phase_actions]) if phase_actions else 0
    
    print(f"\n  Robot position actions:")
    print(f"    Average directional control: {avg_pos_effect:.2e} N")
    print(f"  Phase/interference actions:")
    print(f"    Average directional control: {avg_phase_effect:.2e} N")
    
    if avg_pos_effect > avg_phase_effect * 2:
        print(f"  → Robot MOTION dominates control authority (position >> phase)")
    elif avg_phase_effect > avg_pos_effect * 2:
        print(f"  → PHASE MODULATION dominates control authority (phase >> position)")
    else:
        print(f"  → Both robot motion and phase modulation contribute significantly")
    
    # Individual vs coupled motions
    individual_puck = [m for m in metrics if m.action_name.startswith("MOVE_")]
    coupled_translate = [m for m in metrics if m.action_name.startswith("TRANSLATE_TRAP")]
    
    avg_individual = np.mean([abs(m.max_delta_force_proj) for m in individual_puck]) if individual_puck else 0
    avg_coupled = np.mean([abs(m.max_delta_force_proj) for m in coupled_translate]) if coupled_translate else 0
    
    print(f"\n  Individual puck moves:")
    print(f"    Average directional control: {avg_individual:.2e} N")
    print(f"  Coupled translations (all pucks together):")
    print(f"    Average directional control: {avg_coupled:.2e} N")
    
    if avg_coupled > avg_individual * 1.5:
        print(f"  → Coupled motions are more effective than individual puck moves")
    elif avg_individual > avg_coupled * 1.5:
        print(f"  → Individual puck moves offer finer control than coupled motions")
    else:
        print(f"  → Individual and coupled motions have comparable authority")
    
    print("\n" + "=" * 70)


# =============================================================================
# Main entry point
# =============================================================================

def main():
    parser = argparse.ArgumentParser(description="Control authority diagnostic experiment")
    
    # Physics / grid
    parser.add_argument("--fast", action="store_true", help="Use coarse grid for quick testing")
    
    # Particle position
    parser.add_argument("--particle_x", type=float, default=1.0, help="Particle x position in mm")
    parser.add_argument("--particle_y", type=float, default=1.0, help="Particle y position in mm")
    
    # Macro action parameters
    parser.add_argument("--macro_step_pos_um", type=float, default=50, help="Position step size in µm")
    parser.add_argument("--macro_step_phase_rad", type=float, default=0.15, help="Phase step in radians")
    parser.add_argument("--macro_amplitude_step", type=float, default=0.01, help="Amplitude step")
    
    # Test directions
    parser.add_argument("--directions", type=int, default=8, help="Number of test directions for projection")
    
    # Action set
    parser.add_argument("--all_actions", action="store_true", help="Test ALL macro action types")
    
    # Output
    parser.add_argument("--top_k", type=int, default=5, help="Number of top actions to highlight")
    
    args = parser.parse_args()
    
    print("=" * 70)
    print("CONTROL AUTHORITY DIAGNOSTIC EXPERIMENT")
    print("=" * 70)
    
    # Create timestamped output directory
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    outdir = Path("results/control_authority") / f"run_{timestamp}"
    outdir.mkdir(parents=True, exist_ok=True)
    print(f"\nOutput directory: {outdir}")
    
    # ===== Domain + Physics =====
    if args.fast:
        domain = DishDomain(Lx=2e-3, Ly=2e-3, Nx=80, Ny=80)
    else:
        domain = DishDomain(Lx=2e-3, Ly=2e-3, Nx=100, Ny=100)
    
    medium = MediumProps(f=2e6, c0=1500.0, rho0=1000.0, loss_eta=1e-3, kz=0.0, coupling_alpha=1.0)
    particle = ParticleProps(a=5e-6, rho_p=1050.0, c_p=2350.0)
    
    cfg = EvaluatorConfig(
        sigma_x=0.10e-3,
        sigma_y=0.15e-3,
        bottom_band=0.25e-3,
        dt=5e-3,
        viscosity=1e-3,
        alpha_g=2e3,
        max_step=0.08e-3,
        use_2d_forcing=True,
    )
    
    ev = Evaluator3Pucks(domain, medium, particle, cfg)
    
    print(f"\nPhysics:")
    print(f"  Domain: {domain.Lx*1e3:.1f} x {domain.Ly*1e3:.1f} mm")
    print(f"  Grid: {domain.Nx} x {domain.Ny}")
    print(f"  alpha_g: {cfg.alpha_g:.0e}")
    
    # ===== Fixed baseline state =====
    particle_x = args.particle_x * 1e-3  # mm -> m
    particle_y = args.particle_y * 1e-3
    
    # Initial 3-puck configuration (same as demo_surf_greedy)
    ctrl_baseline = Control3Pucks(
        xA=particle_x - 0.4e-3, yA=0.03e-3, vA=0.08, phiA=0.0,
        xB=particle_x + 0.4e-3, yB=0.03e-3, vB=0.08, phiB=np.pi,
        xC=particle_x, yC=0.20e-3, vC=0.08, phiC=np.pi/2,
    )
    ctrl_baseline = ev.clip_control(ctrl_baseline)
    
    print(f"\nBaseline state:")
    print(f"  Particle: ({particle_x*1e3:.2f}, {particle_y*1e3:.2f}) mm")
    print(f"  Transducers:")
    print(f"    A: ({ctrl_baseline.xA*1e3:.2f}, {ctrl_baseline.yA*1e3:.2f}) mm, v={ctrl_baseline.vA:.3f}, φ={ctrl_baseline.phiA:.2f}")
    print(f"    B: ({ctrl_baseline.xB*1e3:.2f}, {ctrl_baseline.yB*1e3:.2f}) mm, v={ctrl_baseline.vB:.3f}, φ={ctrl_baseline.phiB:.2f}")
    print(f"    C: ({ctrl_baseline.xC*1e3:.2f}, {ctrl_baseline.yC*1e3:.2f}) mm, v={ctrl_baseline.vC:.3f}, φ={ctrl_baseline.phiC:.2f}")
    
    # ===== Compute baseline =====
    print("\nComputing baseline field...")
    t0 = time.perf_counter()
    baseline = compute_baseline(ev, particle_x, particle_y, ctrl_baseline)
    baseline_time = (time.perf_counter() - t0) * 1000.0
    print(f"  Baseline solve: {baseline_time:.1f} ms")
    print(f"  Force at particle: {baseline.force_mag_at_particle:.2e} N")
    print(f"  Trap present: {baseline.trap_present}")
    
    # ===== Test directions =====
    n_dirs = args.directions
    angles = np.linspace(0, 2 * np.pi, n_dirs, endpoint=False)
    test_directions = np.column_stack([np.cos(angles), np.sin(angles)])
    print(f"\nTest directions: {n_dirs} (evenly spaced)")
    
    # ===== Action set =====
    if args.all_actions:
        action_types = list(MacroActionType3Puck)
    else:
        # Default 13-action set (same as demo)
        action_types = [
            MacroActionType3Puck.HOLD,
            MacroActionType3Puck.TRANSLATE_TRAP_X_POS,
            MacroActionType3Puck.TRANSLATE_TRAP_X_NEG,
            MacroActionType3Puck.TRANSLATE_TRAP_Y_POS,
            MacroActionType3Puck.TRANSLATE_TRAP_Y_NEG,
            MacroActionType3Puck.ROTATE_INTERFERENCE_CW,
            MacroActionType3Puck.ROTATE_INTERFERENCE_CCW,
            MacroActionType3Puck.MOVE_A_RIGHT,
            MacroActionType3Puck.MOVE_A_LEFT,
            MacroActionType3Puck.MOVE_B_RIGHT,
            MacroActionType3Puck.MOVE_B_LEFT,
            MacroActionType3Puck.PHASE_SHIFT_B_POS,
            MacroActionType3Puck.PHASE_SHIFT_B_NEG,
        ]
    
    print(f"\nActions to evaluate: {len(action_types)}")
    for a in action_types:
        print(f"  - {a.name}")
    
    # ===== Macro action parameters =====
    macro_magnitude = args.macro_step_pos_um * 1e-6
    macro_phase_step = args.macro_step_phase_rad
    macro_amplitude_step = args.macro_amplitude_step
    
    print(f"\nMacro action parameters:")
    print(f"  Position step: {macro_magnitude*1e6:.0f} µm")
    print(f"  Phase step: {macro_phase_step:.2f} rad")
    print(f"  Amplitude step: {macro_amplitude_step:.3f}")
    
    # ===== Evaluate all actions =====
    print(f"\nEvaluating actions...")
    metrics: list[ActionMetrics] = []
    
    for i, action_type in enumerate(action_types):
        t0 = time.perf_counter()
        m = evaluate_action(
            ev, baseline, action_type,
            macro_magnitude, macro_phase_step, macro_amplitude_step,
            test_directions,
        )
        metrics.append(m)
        elapsed = (time.perf_counter() - t0) * 1000.0
        print(f"  [{i+1:2d}/{len(action_types)}] {action_type.name:30s}  Δ|F|={m.delta_force_mag:+.2e} N  ({elapsed:.0f} ms)")
    
    # ===== Write outputs =====
    print(f"\nWriting outputs...")
    
    csv_path = outdir / "control_authority.csv"
    write_csv(metrics, csv_path)
    print(f"  CSV: {csv_path}")
    
    bars_path = outdir / "summary_bars.png"
    plot_summary_bars(metrics, baseline, bars_path)
    print(f"  Bar charts: {bars_path}")
    
    polar_path = outdir / "direction_polar.png"
    plot_direction_polar(metrics, polar_path)
    print(f"  Polar plot: {polar_path}")
    
    # ===== Print summary =====
    print_summary(metrics, baseline, top_k=args.top_k)
    
    print(f"\nDone. Results saved to: {outdir}")


if __name__ == "__main__":
    main()
