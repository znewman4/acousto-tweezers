#!/usr/bin/env python3
"""
Scale-Calibrated Vortex + Standing Wave Superposition Comparison

Three-case comparison study:
1. Standing wave only (side walls)
2. Vortex only (top aperture)
3. Coherent superposition (both)

With two dish size presets:
- Preset A: 2cm dish, ~500 kHz, 2mm aperture
- Preset B: 3mm dish, ~2 MHz, 0.7mm aperture

Outputs:
- 3D fields exported to VTU (pressure, Gor'kov)
- 2D slices with difference plots
- Particle trajectories in combined field
"""

import argparse
from pathlib import Path
from datetime import datetime
import numpy as np
import matplotlib.pyplot as plt
from mpi4py import MPI
from petsc4py import PETSc

import dolfinx
from dolfinx import fem, mesh, plot
import ufl

import sys
sys.path.append(str(Path(__file__).parents[2] / 'src'))

from acoustweezers.physics.acoustics.vortex_lens import VortexLensConfig, compute_azimuthal_phase, compute_amplitude_profile
from acoustweezers.experiments.square_dish.phase_control import SquareDishConfig


# Material properties (water)
class FluidProperties:
    def __init__(self):
        self.density = 997.0  # kg/m³
        self.sound_speed = 1484.0  # m/s
        self.viscosity = 0.001  # Pa·s


# =============================================================================
# DISH SIZE PRESETS
# =============================================================================

PRESET_A = {
    'name': 'Preset A: 2cm dish, 500 kHz',
    'dish_size_m': 0.02,
    'frequency_hz': 500e3,
    'wavelength_m': 2.968e-3,  # c=1484 m/s water
    'aperture_radius_m': 0.002,  # 2mm
    'aperture_center_xy_m': None,  # None = dish center, or [x, y] in meters
    'vortex_gain': 1.0,  # Velocity amplitude scale
    'standing_gain': 1.0,
    'elements_per_wavelength': 6,
}

PRESET_B = {
    'name': 'Preset B: 3mm dish, 2 MHz',
    'dish_size_m': 0.003,
    'frequency_hz': 2.0e6,
    'wavelength_m': 0.742e-3,  # c=1484 m/s water
    'aperture_radius_m': 0.0007,  # 0.7mm
    'aperture_center_xy_m': None,
    'vortex_gain': 1.0,
    'standing_gain': 1.0,
    'elements_per_wavelength': 6,
}


# =============================================================================
# SOLVER FUNCTIONS
# =============================================================================

def solve_standing_only(preset: dict, fluid: FluidProperties):
    """Solve standing wave (side walls actuation only)."""
    
    L = preset['dish_size_m']
    freq = preset['frequency_hz']
    omega = 2 * np.pi * freq
    k = omega / fluid.sound_speed
    gain = preset['standing_gain']
    
    # Create mesh
    nx = int(L / preset['wavelength_m'] * preset['elements_per_wavelength'])
    domain = mesh.create_box(
        MPI.COMM_WORLD,
        [[0, 0, 0], [L, L, L]],
        [nx, nx, nx],
        cell_type=mesh.CellType.tetrahedron
    )
    
    # Mark boundaries
    def bottom_wall(x): return np.isclose(x[2], 0)
    def top_wall(x): return np.isclose(x[2], L)
    def x_walls(x): return np.logical_or(np.isclose(x[0], 0), np.isclose(x[0], L))
    def y_walls(x): return np.logical_or(np.isclose(x[1], 0), np.isclose(x[1], L))
    
    fdim = domain.topology.dim - 1
    facet_indices, facet_markers = [], []
    for marker, locator in enumerate([bottom_wall, x_walls, y_walls, top_wall], start=1):
        facets = mesh.locate_entities_boundary(domain, fdim, locator)
        facet_indices.append(facets)
        facet_markers.append(np.full_like(facets, marker))
    
    facet_indices = np.hstack(facet_indices).astype(np.int32)
    facet_markers = np.hstack(facet_markers).astype(np.int32)
    sorted_facets = np.argsort(facet_indices)
    facet_tag = mesh.meshtags(domain, fdim, facet_indices[sorted_facets], facet_markers[sorted_facets])
    
    # Function space (complex)
    V = fem.functionspace(domain, ("Lagrange", 2))
    
    # Standing wave actuation on side walls (x and y)
    # Simple uniform velocity v_n = v0 on walls
    v0 = 1e-6 * gain  # 1 μm/s scaled by gain
    
    # Impedance BC parameter
    Z = fluid.density * fluid.sound_speed
    ds = ufl.Measure("ds", domain=domain, subdomain_data=facet_tag)
    
    # Weak form
    u = ufl.TrialFunction(V)
    v = ufl.TestFunction(V)
    
    # Volume terms
    a = (ufl.inner(ufl.grad(u), ufl.grad(v)) / fluid.density - 
         k**2 * ufl.inner(u, v) / fluid.density) * ufl.dx
    
    # Impedance BCs on bottom, top, and sides
    for tag in [1, 2, 3, 4]:  # bottom, x-walls, y-walls, top
        a += -1j * omega * ufl.inner(u, v) / Z * ds(tag)
    
    # Actuation on x-walls (tag 2) and y-walls (tag 3)
    g_standing = -1j * omega * fluid.density * v0
    L_form = g_standing * ufl.inner(1.0, v) * (ds(2) + ds(3))
    
    # Solve
    problem = fem.petsc.LinearProblem(a, L_form, bcs=[], petsc_options={
        "ksp_type": "gmres",
        "ksp_rtol": 1e-8,
        "ksp_max_it": 2000,
        "pc_type": "ilu"
    })
    p = problem.solve()
    
    # Diagnostics
    max_p = np.max(np.abs(p.x.array[:]))
    max_bc = np.abs(g_standing)
    print(f"  [Diagnostics] max|p| = {max_p:.3e} Pa")
    print(f"  [Diagnostics] Standing BC amplitude |g| = {max_bc:.3e} Pa·s/m")
    
    return domain, facet_tag, p


def solve_vortex_only(preset: dict, fluid: FluidProperties, topological_charge: int = 1):
    """Solve vortex lens (top aperture only)."""
    
    L = preset['dish_size_m']
    freq = preset['frequency_hz']
    omega = 2 * np.pi * freq
    k = omega / fluid.sound_speed
    gain = preset['vortex_gain']
    
    # Create mesh
    nx = int(L / preset['wavelength_m'] * preset['elements_per_wavelength'])
    domain = mesh.create_box(
        MPI.COMM_WORLD,
        [[0, 0, 0], [L, L, L]],
        [nx, nx, nx],
        cell_type=mesh.CellType.tetrahedron
    )
    
    # Mark boundaries
    def bottom_wall(x): return np.isclose(x[2], 0)
    def top_wall(x): return np.isclose(x[2], L)
    def x_walls(x): return np.logical_or(np.isclose(x[0], 0), np.isclose(x[0], L))
    def y_walls(x): return np.logical_or(np.isclose(x[1], 0), np.isclose(x[1], L))
    
    fdim = domain.topology.dim - 1
    facet_indices, facet_markers = [], []
    for marker, locator in enumerate([bottom_wall, x_walls, y_walls, top_wall], start=1):
        facets = mesh.locate_entities_boundary(domain, fdim, locator)
        facet_indices.append(facets)
        facet_markers.append(np.full_like(facets, marker))
    
    facet_indices = np.hstack(facet_indices).astype(np.int32)
    facet_markers = np.hstack(facet_markers).astype(np.int32)
    sorted_facets = np.argsort(facet_indices)
    facet_tag = mesh.meshtags(domain, fdim, facet_indices[sorted_facets], facet_markers[sorted_facets])
    
    # Function space (complex)
    V = fem.functionspace(domain, ("Lagrange", 2))
    
    # Vortex configuration
    if preset.get('aperture_center_xy_m') is not None:
        vortex_center = np.array([preset['aperture_center_xy_m'][0], 
                                  preset['aperture_center_xy_m'][1], L])
    else:
        vortex_center = np.array([L/2, L/2, L])
    
    vortex_config = VortexLensConfig(
        topological_charge=topological_charge,
        center=tuple(vortex_center),
        amplitude=1e-6 * gain,  # Velocity amplitude in m/s
        aperture_radius=preset['aperture_radius_m'],
        apodization='cosine_taper',
        axis='z'
    )
    
    # Get top boundary DOFs and coordinates
    top_facets = mesh.locate_entities_boundary(domain, fdim, top_wall)
    top_dofs = fem.locate_dofs_topological(V, fdim, top_facets)
    
    # Extract DOF coordinates
    dof_coords = V.tabulate_dof_coordinates()
    top_coords = dof_coords[top_dofs]
    
    # Compute vortex pattern
    phi_vortex = compute_azimuthal_phase(top_coords, vortex_config, vortex_center)
    amp_vortex = compute_amplitude_profile(top_coords, vortex_config, vortex_center)
    vortex_pattern = amp_vortex * np.exp(1j * phi_vortex)
    
    # Create vortex function
    vortex_func = fem.Function(V)
    vortex_func.x.array[top_dofs] = vortex_pattern
    
    # Impedance BC parameter
    Z = fluid.density * fluid.sound_speed
    ds = ufl.Measure("ds", domain=domain, subdomain_data=facet_tag)
    
    # Weak form
    u = ufl.TrialFunction(V)
    v = ufl.TestFunction(V)
    
    # Volume terms
    a = (ufl.inner(ufl.grad(u), ufl.grad(v)) / fluid.density - 
         k**2 * ufl.inner(u, v) / fluid.density) * ufl.dx
    
    # Impedance BCs on all boundaries
    for tag in [1, 2, 3, 4]:  # bottom, x-walls, y-walls, top
        a += -1j * omega * ufl.inner(u, v) / Z * ds(tag)
    
    # Vortex actuation on top (tag 4)
    g_vortex = -1j * omega * fluid.density * vortex_func
    L_form = ufl.inner(g_vortex, v) * ds(4)
    
    # Solve
    problem = fem.petsc.LinearProblem(a, L_form, bcs=[], petsc_options={
        "ksp_type": "gmres",
        "ksp_rtol": 1e-8,
        "ksp_max_it": 2000,
        "pc_type": "ilu"
    })
    p = problem.solve()
    
    # Diagnostics
    max_p = np.max(np.abs(p.x.array[:]))
    max_bc_vortex = np.max(np.abs(vortex_pattern))
    print(f"  [Diagnostics] max|p| = {max_p:.3e} Pa")
    print(f"  [Diagnostics] Vortex BC max|pattern| = {max_bc_vortex:.3e}")
    print(f"  [Diagnostics] Vortex BC active DOFs = {np.sum(np.abs(vortex_pattern) > 1e-12)} / {len(vortex_pattern)}")
    
    return domain, facet_tag, p


def solve_combined(preset: dict, fluid: FluidProperties, topological_charge: int = 1):
    """Solve coherent superposition (standing + vortex)."""
    
    L = preset['dish_size_m']
    freq = preset['frequency_hz']
    omega = 2 * np.pi * freq
    k = omega / fluid.sound_speed
    v_gain = preset['vortex_gain']
    s_gain = preset['standing_gain']
    
    # Create mesh
    nx = int(L / preset['wavelength_m'] * preset['elements_per_wavelength'])
    domain = mesh.create_box(
        MPI.COMM_WORLD,
        [[0, 0, 0], [L, L, L]],
        [nx, nx, nx],
        cell_type=mesh.CellType.tetrahedron
    )
    
    # Mark boundaries
    def bottom_wall(x): return np.isclose(x[2], 0)
    def top_wall(x): return np.isclose(x[2], L)
    def x_walls(x): return np.logical_or(np.isclose(x[0], 0), np.isclose(x[0], L))
    def y_walls(x): return np.logical_or(np.isclose(x[1], 0), np.isclose(x[1], L))
    
    fdim = domain.topology.dim - 1
    facet_indices, facet_markers = [], []
    for marker, locator in enumerate([bottom_wall, x_walls, y_walls, top_wall], start=1):
        facets = mesh.locate_entities_boundary(domain, fdim, locator)
        facet_indices.append(facets)
        facet_markers.append(np.full_like(facets, marker))
    
    facet_indices = np.hstack(facet_indices).astype(np.int32)
    facet_markers = np.hstack(facet_markers).astype(np.int32)
    sorted_facets = np.argsort(facet_indices)
    facet_tag = mesh.meshtags(domain, fdim, facet_indices[sorted_facets], facet_markers[sorted_facets])
    
    # Function space (complex)
    V = fem.functionspace(domain, ("Lagrange", 2))
    
    # Vortex configuration
    if preset.get('aperture_center_xy_m') is not None:
        vortex_center = np.array([preset['aperture_center_xy_m'][0], 
                                  preset['aperture_center_xy_m'][1], L])
    else:
        vortex_center = np.array([L/2, L/2, L])
    
    vortex_config = VortexLensConfig(
        topological_charge=topological_charge,
        center=tuple(vortex_center),
        amplitude=1e-6 * v_gain,
        aperture_radius=preset['aperture_radius_m'],
        apodization='cosine_taper',
        axis='z'
    )
    
    # Get top boundary DOFs and coordinates
    top_facets = mesh.locate_entities_boundary(domain, fdim, top_wall)
    top_dofs = fem.locate_dofs_topological(V, fdim, top_facets)
    
    # Extract DOF coordinates
    dof_coords = V.tabulate_dof_coordinates()
    top_coords = dof_coords[top_dofs]
    
    # Compute vortex pattern
    phi_vortex = compute_azimuthal_phase(top_coords, vortex_config, vortex_center)
    amp_vortex = compute_amplitude_profile(top_coords, vortex_config, vortex_center)
    vortex_pattern = amp_vortex * np.exp(1j * phi_vortex)
    
    # Create vortex function (initialize to zero everywhere)
    vortex_func = fem.Function(V)
    vortex_func.x.array[:] = 0.0  # Zero everywhere
    vortex_func.x.array[top_dofs] = vortex_pattern  # Set vortex on top DOFs
    
    # Impedance BC parameter
    Z = fluid.density * fluid.sound_speed
    ds = ufl.Measure("ds", domain=domain, subdomain_data=facet_tag)
    
    # Weak form
    u = ufl.TrialFunction(V)
    v = ufl.TestFunction(V)
    
    # Volume terms
    a = (ufl.inner(ufl.grad(u), ufl.grad(v)) / fluid.density - 
         k**2 * ufl.inner(u, v) / fluid.density) * ufl.dx
    
    # Impedance BCs on all boundaries
    for tag in [1, 2, 3, 4]:
        a += -1j * omega * ufl.inner(u, v) / Z * ds(tag)
    
    # Combined actuation
    v0_standing = 1e-6 * s_gain
    g_standing = -1j * omega * fluid.density * v0_standing
    g_vortex = -1j * omega * fluid.density * vortex_func
    
    L_form = (g_standing * ufl.inner(1.0, v) * (ds(2) + ds(3)) +  # Standing on sides
              ufl.inner(g_vortex, v) * ds(4))  # Vortex on top
    
    # Solve
    problem = fem.petsc.LinearProblem(a, L_form, bcs=[], petsc_options={
        "ksp_type": "gmres",
        "ksp_rtol": 1e-8,
        "ksp_max_it": 2000,
        "pc_type": "ilu"
    })
    p = problem.solve()
    
    return domain, facet_tag, p


# =============================================================================
# VISUALIZATION AND EXPORT
# =============================================================================

def plot_slice_comparison(p_standing, p_vortex, p_combined, domain, preset, output_dir):
    """Plot 2D slices comparing three cases."""
    
    L = preset['dish_size_m']
    
    # Evaluation grid (mid-height)
    nx_grid = 150
    x_grid = np.linspace(0, L, nx_grid)
    y_grid = np.linspace(0, L, nx_grid)
    X, Y = np.meshgrid(x_grid, y_grid)
    z_mid = L / 2
    points = np.column_stack([X.ravel(), Y.ravel(), np.full(X.size, z_mid)])
    
    # Evaluate pressure fields
    bb_tree = dolfinx.geometry.bb_tree(domain, domain.topology.dim)
    p_stand_vals = np.zeros(points.shape[0], dtype=np.complex128)
    p_vortex_vals = np.zeros(points.shape[0], dtype=np.complex128)
    p_comb_vals = np.zeros(points.shape[0], dtype=np.complex128)
    
    for func, vals in [(p_standing, p_stand_vals), (p_vortex, p_vortex_vals), (p_combined, p_comb_vals)]:
        cells = []
        points_on_proc = []
        cell_candidates = dolfinx.geometry.compute_collisions_points(bb_tree, points)
        colliding_cells = dolfinx.geometry.compute_colliding_cells(domain, cell_candidates, points)
        for i, point in enumerate(points):
            if len(colliding_cells.links(i)) > 0:
                points_on_proc.append(point)
                cells.append(colliding_cells.links(i)[0])
        
        if len(points_on_proc) > 0:
            points_on_proc = np.array(points_on_proc, dtype=np.float64)
            eval_result = func.eval(points_on_proc, cells)
            # Handle shape (N,1) -> (N,)
            if eval_result.ndim == 2:
                eval_result = eval_result[:, 0]
            vals[range(len(points_on_proc))] = eval_result
    
    p_stand_mag = np.abs(p_stand_vals).reshape(X.shape)
    p_vortex_mag = np.abs(p_vortex_vals).reshape(X.shape)
    p_comb_mag = np.abs(p_comb_vals).reshape(X.shape)
    
    # Plot
    fig, axes = plt.subplots(1, 3, figsize=(15, 4.5))
    
    vmax = max(np.max(p_stand_mag), np.max(p_vortex_mag), np.max(p_comb_mag))
    
    im1 = axes[0].contourf(X*1e3, Y*1e3, p_stand_mag, levels=50, cmap='viridis', vmin=0, vmax=vmax)
    axes[0].set_title('Standing Wave Only')
    axes[0].set_xlabel('x (mm)')
    axes[0].set_ylabel('y (mm)')
    plt.colorbar(im1, ax=axes[0], label='|p| (Pa)')
    
    im2 = axes[1].contourf(X*1e3, Y*1e3, p_vortex_mag, levels=50, cmap='viridis', vmin=0, vmax=vmax)
    axes[1].set_title(f'Vortex Only (ℓ={preset.get("topological_charge", 1)})')
    axes[1].set_xlabel('x (mm)')
    axes[1].set_ylabel('y (mm)')
    plt.colorbar(im2, ax=axes[1], label='|p| (Pa)')
    
    im3 = axes[2].contourf(X*1e3, Y*1e3, p_comb_mag, levels=50, cmap='viridis', vmin=0, vmax=vmax)
    axes[2].set_title('Combined (Coherent Sum)')
    axes[2].set_xlabel('x (mm)')
    axes[2].set_ylabel('y (mm)')
    plt.colorbar(im3, ax=axes[2], label='|p| (Pa)')
    
    plt.tight_layout()
    output_path = output_dir / 'pressure_comparison_slice.png'
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"[Plot] Saved {output_path}")
    plt.close()


def plot_difference_fields(p_standing, p_combined, domain, preset, output_dir):
    """Plot difference: combined - standing."""
    
    L = preset['dish_size_m']
    
    # Evaluation grid
    nx_grid = 150
    x_grid = np.linspace(0, L, nx_grid)
    y_grid = np.linspace(0, L, nx_grid)
    X, Y = np.meshgrid(x_grid, y_grid)
    z_mid = L / 2
    points = np.column_stack([X.ravel(), Y.ravel(), np.full(X.size, z_mid)])
    
    # Evaluate
    bb_tree = dolfinx.geometry.bb_tree(domain, domain.topology.dim)
    p_stand_vals = np.zeros(points.shape[0], dtype=np.complex128)
    p_comb_vals = np.zeros(points.shape[0], dtype=np.complex128)
    
    for func, vals in [(p_standing, p_stand_vals), (p_combined, p_comb_vals)]:
        cells = []
        points_on_proc = []
        cell_candidates = dolfinx.geometry.compute_collisions_points(bb_tree, points)
        colliding_cells = dolfinx.geometry.compute_colliding_cells(domain, cell_candidates, points)
        for i, point in enumerate(points):
            if len(colliding_cells.links(i)) > 0:
                points_on_proc.append(point)
                cells.append(colliding_cells.links(i)[0])
        
        if len(points_on_proc) > 0:
            points_on_proc = np.array(points_on_proc, dtype=np.float64)
            eval_result = func.eval(points_on_proc, cells)
            # Handle shape (N,1) -> (N,)
            if eval_result.ndim == 2:
                eval_result = eval_result[:, 0]
            vals[range(len(points_on_proc))] = eval_result
    
    # Difference
    diff_vals = np.abs(p_comb_vals) - np.abs(p_stand_vals)
    diff_grid = diff_vals.reshape(X.shape)
    
    # Plot with diverging colormap
    fig, ax = plt.subplots(figsize=(6, 5))
    vmax_diff = np.max(np.abs(diff_grid))
    im = ax.contourf(X*1e3, Y*1e3, diff_grid, levels=50, cmap='RdBu_r', 
                     vmin=-vmax_diff, vmax=vmax_diff)
    ax.set_title('Pressure Difference: |p|_combined - |p|_standing')
    ax.set_xlabel('x (mm)')
    ax.set_ylabel('y (mm)')
    plt.colorbar(im, ax=ax, label='Δ|p| (Pa)')
    
    output_path = output_dir / 'pressure_difference.png'
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"[Plot] Saved {output_path}")
    plt.close()


def export_to_vtu(p_func, domain, filename):
    """Export pressure field to VTU format."""
    from dolfinx.io import VTXWriter
    
    # Create magnitude function for export (real-valued)
    V = p_func.function_space
    V_real = fem.functionspace(domain, ("Lagrange", 2))
    
    p_mag = fem.Function(V_real)
    p_mag.x.array[:] = np.abs(p_func.x.array[:])
    p_mag.name = "pressure_magnitude"
    
    # Write to BP4 format (readable by ParaView)
    with VTXWriter(domain.comm, filename, [p_mag], engine="BP4") as vtx:
        vtx.write(0.0)
    
    print(f"[Export] Saved {filename}")


# =============================================================================
# MAIN
# =============================================================================

def main():
    parser = argparse.ArgumentParser(description="Vortex + Standing Wave Comparison")
    parser.add_argument('--preset', type=str, choices=['A', 'B'], default='A',
                       help="Dish size preset: A (2cm) or B (3mm)")
    parser.add_argument('--topological_charge', type=int, default=1,
                       help="Vortex topological charge ℓ")
    parser.add_argument('--output_dir', type=str, default=None,
                       help="Output directory (default: results/comparison_<preset>_<timestamp>)")
    
    args = parser.parse_args()
    
    # Select preset
    preset = PRESET_A if args.preset == 'A' else PRESET_B
    preset['topological_charge'] = args.topological_charge
    
    # Output directory
    if args.output_dir:
        output_dir = Path(args.output_dir)
    else:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_dir = Path(__file__).parents[2] / 'results' / f'comparison_{args.preset}_{timestamp}'
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"\n{'='*60}")
    print(f"VORTEX + STANDING WAVE COMPARISON")
    print(f"{'='*60}")
    print(f"Preset: {preset['name']}")
    print(f"Topological charge: ℓ = {args.topological_charge}")
    print(f"Dish size: {preset['dish_size_m']*1e3:.1f} mm")
    print(f"Frequency: {preset['frequency_hz']/1e6:.2f} MHz")
    print(f"Wavelength: {preset['wavelength_m']*1e3:.3f} mm")
    print(f"Aperture: {preset['aperture_radius_m']*1e3:.2f} mm")
    print(f"Output: {output_dir}")
    print(f"{'='*60}\n")
    
    # Fluid properties
    fluid = FluidProperties()
    
    # Solve three cases
    print("[1/3] Solving standing wave only...")
    domain_s, facet_s, p_standing = solve_standing_only(preset, fluid)
    print(f"      max|p| = {np.max(np.abs(p_standing.x.array)):.3e} Pa")
    
    print("[2/3] Solving vortex only...")
    domain_v, facet_v, p_vortex = solve_vortex_only(preset, fluid, args.topological_charge)
    print(f"      max|p| = {np.max(np.abs(p_vortex.x.array)):.3e} Pa")
    
    print("[3/3] Solving combined...")
    domain_c, facet_c, p_combined = solve_combined(preset, fluid, args.topological_charge)
    print(f"      max|p| = {np.max(np.abs(p_combined.x.array)):.3e} Pa")
    
    # Visualization
    print("\n[Viz] Generating comparison plots...")
    plot_slice_comparison(p_standing, p_vortex, p_combined, domain_c, preset, output_dir)
    plot_difference_fields(p_standing, p_combined, domain_c, preset, output_dir)
    
    # Export to VTU
    print("\n[Export] Writing VTU files...")
    export_to_vtu(p_standing, domain_s, output_dir / 'standing_only.bp')
    export_to_vtu(p_vortex, domain_v, output_dir / 'vortex_only.bp')
    export_to_vtu(p_combined, domain_c, output_dir / 'combined.bp')
    
    print(f"\n{'='*60}")
    print(f"DONE. Results in: {output_dir}")
    print(f"{'='*60}\n")


if __name__ == '__main__':
    main()
