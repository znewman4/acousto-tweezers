...
## 10  Hybrid FEM–ASM Bridge Formation Workflow

This is the **primary research workflow** for the project. It investigates
whether interference between a focused vortex beam and the standing-wave
lattice can reduce the Gor'kov potential barrier between adjacent traps —
forming a "bridge" that enables controlled particle transport.

### 10.1  Method Overview

The workflow combines two complementary solvers:

1. **FEM standing-wave fields** — A full 3D finite-element solve of the
   Helmholtz equation in the petri-dish cavity produces the periodic
   pressure lattice. The result is cached to disk as a `.npz` file
   (`results/fem_standing_wave_cache/standing_wave_epl6.npz`, 762K DOFs)
   and reused across all studies without re-solving.

2. **ASM vortex propagation** — A focused orbital-angular-momentum beam is
   generated analytically at the transducer plane and propagated to the
   trap plane using the angular spectrum method (ASM). This is orders of
   magnitude faster than FEM and allows rapid parameter sweeps.

3. **Linear superposition** — The two complex pressure fields are combined:

   $$p_\text{total}(x,y) = p_\text{stand}(x,y) + \alpha\,e^{i\varphi_0}\,\hat{p}_\text{vort}(x - x_0,\, y - y_0)$$

   where $\alpha$ controls amplitude, $\varphi_0$ controls relative phase,
   and $(x_0, y_0)$ shifts the vortex laterally relative to the trap
   lattice. This superposition is valid under the linear Helmholtz equation.

4. **Gor'kov potential** — The time-averaged radiation force potential is
   computed for polystyrene microspheres:

   $$U = V_p\!\left(f_1 E_\text{pot} - \tfrac{3}{2} f_2 E_\text{kin}\right)$$

   Traps sit at local minima of $U$. A saddle between adjacent minima
   represents the barrier that must be overcome for particle transport.

5. **Bridge metric** — For two adjacent trap minima the barrier height is:

   $$B = U_\text{saddle} - \max(U_{\min1},\, U_{\min2})$$

   $B > 0$ means separate traps; $B \le 0$ means a bridge (connected basin).

### 10.2  Physical Reasoning

| Component | Justification |
|-----------|---------------|
| FEM for standing waves | Accurately captures cavity resonances, wall reflections, and PML boundaries |
| ASM for vortex | Efficient FFT-based propagation; the vortex is generated by a single transducer with no cavity interaction |
| Superposition | Valid because both fields satisfy the same linear Helmholtz equation in water |
| Phase offset $\varphi_0$ | Controls constructive/destructive interference pattern between the two fields |
| Lateral offset $(x_0, y_0)$ | Positions the vortex ring between traps rather than on top of one |

### 10.3  Current Working Configuration

| Parameter | Value |
|-----------|-------|
| Frequency | 2 MHz |
| Water speed of sound | 1484 m/s |
| Wavelength $\lambda$ | 0.742 mm |
| Trap spacing $\lambda/2$ | 0.371 mm |
| Trap plane $z^*$ | 4.190 mm |
| FEM cache | `standing_wave_epl6.npz` (762,129 DOFs) |
| Vortex $\ell$ | 1 |
| Vortex focal length | 4.2 mm |
| Vortex aperture radius | 7.0 mm |
| Apodization | uniform |
| Numerical aperture | 0.86 |
| Vortex waist diameter | 0.618 mm ≈ 1.67 × $\lambda/2$ |
| Particle | polystyrene 5 µm, $f_1 = 0.62$, $f_2 = 0.03$ |

### 10.4  Running the Pipeline

```bash
# Single command — full bridge formation study
python scripts/run_bridge_pipeline.py

# Or run the sweep script directly
python scripts/dev/bridge_phase_offset_study.py
```

The pipeline loads the FEM cache, generates the ASM vortex, sweeps over
$(\alpha, \varphi_0, x_0, y_0)$, computes Gor'kov potentials and bridge
metrics, and produces figures + `REPORT.md` + `stats.json`.

Outputs: `results/bridge_master_study_<timestamp>/`

### 10.5  Key Findings (Preliminary)

- Phase offset $\varphi_0 = \pi$ (anti-phase) consistently minimises the barrier
- Lateral offsets of $+0.2 \times \lambda/2$ in both $x$ and $y$ are optimal
- At $\alpha = 0.20$: barrier reduced by **54%** (from 14.3% to 6.6% of trap depth)
- No true bridge ($B \le 0$) found yet at $\alpha \le 0.20$
- Larger $\alpha$ values and finer offset grids are being explored
