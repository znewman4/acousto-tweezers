...
from .pml import (
    build_pml_stretch_dg0, helmholtz_anisotropic_pml_forms,  # Legacy x-only
    build_pml_stretch_tensor_dg0, helmholtz_tensor_pml_forms  # Production tensor
)
...
class AcousticSolver:
    ...
    def _log_pml_diagnostics(self, pml_stats: dict, bbox: tuple, thickness: float):
        """
        Log PML diagnostics for verification.
        
        Parameters
        ----------
        pml_stats : dict
            Statistics from build_pml_stretch_tensor_dg0
        bbox : tuple
            Mesh bounding box (x_min, x_max, y_min, y_max, z_min, z_max)
        thickness : float
            PML thickness
        """
        if self.mesh.comm.rank == 0:
            print("\n" + "=" * 60)
            print("PML DIAGNOSTICS")
            print("=" * 60)
            print(f"  Bbox: x=[{bbox[0]:.3f}, {bbox[1]:.3f}], "
                  f"y=[{bbox[2]:.3f}, {bbox[3]:.3f}], z=[{bbox[4]:.3f}, {bbox[5]:.3f}]")
            print(f"  Thickness: {thickness*1000:.3f} mm")
            print(f"  Water cells: {pml_stats.get('num_water_cells', 0)}")
            print(f"  PML cells: {pml_stats.get('num_pml_cells', 0)}")
            print()
            
            # Im(s) in water (should be ~0)
            if 'im_s_x_water' in pml_stats:
                print("  Im(s) in WATER (should be ~0):")
                for axis in ['x', 'y', 'z']:
                    key = f'im_s_{axis}_water'
                    if key in pml_stats:
                        stats = pml_stats[key]
                        print(f"    s_{axis}: min={stats['min']:.2e}, "
                              f"med={stats['median']:.2e}, max={stats['max']:.2e}")
                print()
            
            # Im(s) in PML (should be >0)
            if 'im_s_x_pml' in pml_stats:
                print("  Im(s) in PML (should be >0):")
                for axis in ['x', 'y', 'z']:
                    key = f'im_s_{axis}_pml'
                    if key in pml_stats:
                        stats = pml_stats[key]
                        print(f"    s_{axis}: min={stats['min']:.2e}, "
                              f"med={stats['median']:.2e}, max={stats['max']:.2e}")
    ...
    def solve(self, 
        ...
        #
        # NOTE: This form is for standard Helmholtz. If volumetric PML is active,
        # it will be replaced by the anisotropic PML form below.
        a = (
        ...
        # Add volumetric PML or fallback to absorbing boundary condition
        # 
        # If PhysicsLevel >= ACOUSTICS_PML and PML volumes exist in mesh,
        # use anisotropic coordinate-stretching PML for water+PML regions.
        # Other domains (air, dish) use standard Helmholtz.
        if self.config.physics_level.value >= PhysicsLevel.ACOUSTICS_PML:
            # Check if PML volumes exist in the mesh
            pml_water_tag = Domain.PML_WATER.gmsh_tag
            pml_cells = self.cell_tags.find(pml_water_tag)
            
            if len(pml_cells) > 0:
                # Use volumetric 6-face tensor PML for water+PML regions
                #
                # Replaces standard Helmholtz with coordinate-stretching PML:
                #   s_x, s_y, s_z = 1 + i*σ_x,y,z(d)/ω  (complex stretches per axis)
                #
                # The tensor PML form for water+PML regions:
                #   a(p,v) = (1/ρ) * [(1/s_x)*∂p/∂x*∂v̄/∂x + (1/s_y)*∂p/∂y*∂v̄/∂y + (1/s_z)*∂p/∂z*∂v̄/∂z]
                #            - (k²/ρ) * (s_x*s_y*s_z) * p * v̄
                #
                # Non-water domains (air, dish, etc.) keep standard Helmholtz form.
                
                # Get water acoustic properties
                ...
                pml_power = self.config.geometry.pml_stretch_order
                sigma_max = self.config.geometry.pml_max_sigma * omega
                
                # Get mesh bounding box for tensor PML
                bbox_coords = self.mesh.geometry.x
                x_min = float(np.min(bbox_coords[:, 0]))
                x_max = float(np.max(bbox_coords[:, 0]))
                y_min = float(np.min(bbox_coords[:, 1]))
                y_max = float(np.max(bbox_coords[:, 1]))
                z_min = float(np.min(bbox_coords[:, 2]))
                z_max = float(np.max(bbox_coords[:, 2]))
                bbox = (x_min, x_max, y_min, y_max, z_min, z_max)
                
                # Detect all PML tags (may have multiple regions)
                pml_tags = [pml_water_tag]  # Start with water PML
                # Could extend to other PML regions if they exist:
                # for domain in Domain:
                #     if "PML" in domain.name and domain != Domain.PML_WATER:
                #         pml_tags.append(domain.gmsh_tag)
                
                # Build tensor PML stretch fields (DG0)
                tag_water = Domain.WATER.gmsh_tag
                ...
                # Log PML diagnostics
                self._log_pml_diagnostics(pml_stats, bbox, pml_thickness)
                
                # Create domain-specific measures for water and PML
                dx_water = Measure("dx", domain=self.mesh, subdomain_data=self.cell_tags, 
                                  subdomain_id=tag_water)
                dx_pml = Measure("dx", domain=self.mesh, subdomain_data=self.cell_tags, 
                                subdomain_id=pml_water_tag)
                
                # Build tensor PML form for water+PML using production module
                a_water_pml, _ = helmholtz_tensor_pml_forms(
                    p, v, self.mesh, k, rho_water, omega,
                    s_x, s_y, s_z, s_x_inv, s_y_inv, s_z_inv,
                    dx_water, dx_pml,
                    source_form=None
                )
                
                # Build standard Helmholtz form for other domains (air, dish, bath)
                # Use domain-specific measures (do not double-count water+PML)
                other_domains = [Domain.AIR, Domain.DISH_BOTTOM, Domain.BATH_FLUID]
                a_other = None
                for dom in other_domains:
                    cells = self.cell_tags.find(dom.gmsh_tag)
                    if len(cells) > 0:
                        dx_dom = Measure("dx", domain=self.mesh, subdomain_data=self.cell_tags,
                                        subdomain_id=dom.gmsh_tag)
                        a_dom = (
                            inner(self.inv_rho * grad(p), grad(v)) * dx_dom
                            - (omega**2 / self.K) * inner(p, v) * dx_dom
                        )
                        a_other = a_dom if a_other is None else a_other + a_dom
                
                # Combine: a = a_water_pml + a_other
                ...
                # Fallback: First-order absorbing BC (Sommerfeld radiation condition)
                # (No PML volumes in mesh - use ABC on outer boundary)
                c_water = self.materials.water.sound_speed
                k = omega / c_water
